...
#Creating tabs
tab1, tab2 ,tab3, tab4, tab5= st.tabs(['**Transaction Analysis for Market Expansion**','**User Registration Analysis**','**Insurance Transactions Analysis**','**User Engagement and Growth Strategy**','**Insurance Penetration and Growth Potential Analysis**'])
...
with tab1:
    ...
    if tr_state == 'All':
        ...
        with col4:
            ...
            WHERE "Year"='{tr_yr}' AND "Quarter" = '{tr_qtr}' ORDER BY "Count" DESC LIMIT 10;
"""
...
"""
    ...
    # -------------------------       /     State wise Transaction        /        ------------------ #
        ...
        with col3:
            ...
            st.markdown(":violet[**Top 10 Districts**]")
            query_tp2=f"""SELECT "District", "Count" FROM  public."aggTopTransaction"
            WHERE "Year"='{tr_yr}' AND "Quarter" = '{tr_qtr}' AND "State" = '{tr_state}' 
        ...
        with col4:
            ...
            WHERE "Year"='{tr_yr}' AND "State" = '{tr_state}'
